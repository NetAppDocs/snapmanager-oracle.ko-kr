...
Microsoft WSFC(Windows Server Failover Cluster) 및 MSCS(Microsoft Cluster Server) 환경에서 페일오버가 발생하면 서비스(가상) 주소가 활성 호스트에 매핑되고 SnapManager 스케줄이 활성 SnapManager 호스트로 조정되도록 Oracle 서버용 SnapManager를 다시 시작해야 합니다.


NOTE: 동일한 프로필 및 스케줄 이름이 다른 리포지토리에 있는 경우 해당 리포지토리에서 백업 예약 작업이 시작되지 않습니다. 작업이 이미 실행 중입니다. 라는 메시지와 함께 작업이 종료됩니다.

. 다음 명령을 입력합니다. smo schedule create-profile profile_name {[-full{-online|-offline|-auto} [-retain[-hourly|-daily|-weekly|-weekly|-monthly|-limited] [-data[-filesfiles [files]]|[-online|-offline], weekly-common_comment2 |-untilSCNuntscn|-until-dateyyyy-mm-dd HH:mm:ss|-before{-months|-weeks|-hours}}- prune-destprune_dest1, prune_destn2]- schedule-nameding_name[-schedule-commenting_casingforce]- evently-caskeventeedly-cruneedays|- vasingtime-castime-
+
|===
